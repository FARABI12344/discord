...
class AutoPromo(commands.Cog):
    ...
    async def _common_start(self, ctx: commands.Context, reverse: bool):
        ...
        await ctx.send(
            ...
            f"({'reverse' if reverse else 'forward'}) – first run now."
        )
    ...
    async def stop(self, ctx):
        ...
        # IMPORTANT:
        # reload the newest channels.json before saving anything
        self.data = load_data()
        ...
        try:
            ...
            while self.data.get("auto"):
                ...
                delay = max(
                    0,
                    (
                        datetime.fromisoformat(self.data["next_run"])
                        - utc_now()
                    ).total_seconds()
                )
    ...
    async def _run_cycle(self):
        ...
        await log(
            f"▶️ Cycle started "
            f"({'reverse' if self.data.get('reverse') else 'forward'}) "
